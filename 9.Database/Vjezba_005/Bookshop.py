...
def add_knjiga(session, ime_autora, prezime_autora, naslov_knjige, naziv_izdavaca):
    ...
    izdavac = (
        session.query(Izdavac)
        .filter(Izdavac.naziv == naziv_izdavaca)
        .one_or_none()
    )
...
def main():
    db_engine = db.create_engine("sqlite:///9.Database/Vjezba_005/BookshopDB.db")
    ...
    for knjiga in nove_knjige:
        add_knjiga(session, knjiga[0], knjiga[1], knjiga[2], knjiga[3])
